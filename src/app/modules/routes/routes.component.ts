...
export class RoutesComponent implements OnInit {
  ...
  calculateTimes(){
    ...
    //для всех маршрутов
    for(let routeNumber in this.routes){
      //3 смены
      for(let workOrder in this.workOrder){
        //транспорта поедет за смену столько:
        let transportCount = this.routes[routeNumber].transportNeeded;
        let totalWorkTimeInMinutes = 420; //7h
        //шаг между рейсами
        let timeBetweenBuses = totalWorkTimeInMinutes/transportCount;
        ...
        //делаем тайминги вперед:
          ...
          for(let stop in this.routes[routeNumber].stopsList){
